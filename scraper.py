...
for url in list_links:
    ...
    product_dict['nutritional info']['unit'].append(nutritional_table_rows[0].find_elements(by=By.XPATH, value='.//th[@scope="col"]')[1].text)
    product_dict['nutritional info']['energy kJ'].append(nutritional_table_rows[1].find_elements(by=By.XPATH, value='.//td')[0].text)
    product_dict['nutritional info']['energy kcal'].append(nutritional_table_rows[2].find_elements(by=By.XPATH, value='.//td')[0].text)
    
    # Fills dictionary with 0g in case the category doesn't appear on the page
    product_dict['nutritional info']['fat'].append("0g")
    product_dict['nutritional info']['saturates'].append("0g")
    product_dict['nutritional info']['carbohydrate'].append("0g")
    product_dict['nutritional info']['sugars'].append("0g")
    product_dict['nutritional info']['fibre'].append("0g")
    product_dict['nutritional info']['protein'].append("0g")
    product_dict['nutritional info']['salt'].append("0g")


    # Iterates through all of the row headings in the table and updates the relevant key upon matching
